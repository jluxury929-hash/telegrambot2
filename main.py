...
async def main_chat_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == '🚀 Start Trading':
        ...
        await update.message.reply_text("🎯 **MARKET SELECTION**", reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
    elif text == '⚙️ Settings':
        await settings_menu(update, context)
    elif text == '💰 Wallet':
        bal = w3.from_wei(w3.eth.get_balance(vault.address), 'ether')
        price = get_pol_price()
        await update.message.reply_text(f"💳 **Balance:** {bal:.4f} POL (`${float(bal)*price:.2f} USD`)")
    elif text == '📤 Withdraw':
        ...
        await update.message.reply_text("🕴️ **Assistant:** I am monitoring the Polygon mempool for atomic bundle opportunities. Your current stake is **$" + str(context.user_data.get('stake', 10)) + "**.", parse_mode='Markdown')
