 ...
 * SECURITY: Fee Guard Protection + Automatic Profit Cold-Sweep + RugCheck Shield
 * ===============================================================================
 */
...
let SYSTEM = {
    ...
    jitoTip: 2000000, minLiquidity: 15000, velocityThreshold: 1.8,
...
// --- 3. CALLBACK HANDLER (DYNAMIC CYCLING) ---
bot.on('callback_query', async (query) => {
    ...
    if (data === "cycle_risk") {
        ...
        if (SYSTEM.autoPilot) {
            bot.sendMessage(chatId, "🚀 **AUTO-PILOT ACTIVE.** Initializing Sniper Threads...");
...
// --- 4. AUTO-PILOT ENGINE (v9032 PARALLEL LOOP) ---
...
// --- 5. EXECUTION CORE (JUPITER V6 + JITO ATOMIC) ---
async function executeSolShotgun(chatId, addr, symbol) {
    try {
        ...
        if (sig) {
            bot.sendMessage(chatId, `💰 **BOUGHT:** $${symbol}\nSig: \`${sig.slice(0,8)}...\``);
...
async function startIndependentPeakMonitor(chatId, netKey, pos) {
    try {
        ...
        if (pnl >= tp || pnl <= sl) {
            bot.sendMessage(chatId, `📉 **EXIT:** ${pos.symbol} at ${pnl.toFixed(2)}% PnL.`);
