...
/**
 * 🔱 GHOST INJECTION: FLUID RISK, TERMS, WITHDRAWAL & PnL SENSORS
...
async function executeAtomicWithdrawal(chatId) {
    try {
        ...
        if (bal <= buffer) return bot.sendMessage(chatId, "❌ <b>Insufficient SOL for fees.</b>", { parse_mode: 'HTML' });
        ...
        if (res.data.result) {
            bot.sendMessage(chatId, `✅ <b>WITHDRAW SUCCESS:</b> Funds converted and secured in USDC.\n📜 <a href="https://solscan.io/tx/${res.data.result}">Receipt</a>`, { parse_mode: 'HTML' });
        }
    } catch (e) { bot.sendMessage(chatId, "❌ <b>Withdrawal Failed:</b> Insufficient market liquidity for atomic swap."); }
}

// 4. NON-STICKY CALLBACK HANDLER (Acknowledge instantly)
bot.on('callback_query', async (query) => {
    const { data, message, id } = query;
    const chatId = message.chat.id;
    
    // Kill the Telegram loading spinner instantly (Fixes Sticky Buttons)
    ...
    if (data === "cmd_withdraw") {
        ...
        if (SYSTEM.autoPilot) {
            bot.sendMessage(chatId, "🚀 <b>AUTO-PILOT ACTIVE:</b> Monitoring institutional leads.", { parse_mode: 'HTML' });
            Object.keys(NETWORKS).forEach(net => startNetworkSniper(chatId, net));
        } else {
            bot.sendMessage(chatId, "🛑 <b>AUTO-PILOT STOPPED.</b>", { parse_mode: 'HTML' });
        }
