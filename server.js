/**
 * ===============================================================================
 * APEX PREDATOR: NEURAL ULTRA v9077 (GLOBAL ULTIMATUM - REFINED)
 * ===============================================================================
 * Infrastructure: Binance WebSocket + Yellowstone gRPC + Jito Atomic Bundles
 * Features: Dynamic Risk/Term Toggles & Fixed Balance Withdrawal
 * ===============================================================================
 */
...
let SYSTEM = {
    autoPilot: false, tradeAmount: "0.1", risk: 'MAX', term: 'SCALP',
...
// --- 🛠️ WITHDRAWAL SYSTEM (FIXED) ---
async function handleWithdrawal(chatId, destAddr) {
    if (!solWallet) return bot.sendMessage(chatId, "❌ Connect wallet first.");
    try {
        const conn = new Connection(NETWORKS.SOL.endpoints[0], 'confirmed');
        const balance = await conn.getBalance(solWallet.publicKey);
        const fee = 5000; 
        const amount = balance - fee - 100000; // Buffer for tip/rent

        if (amount <= 0) return bot.sendMessage(chatId, "⚠️ Balance too low to cover fees.");
        ...
        bot.sendMessage(chatId, `🏦 <b>WITHDRAWAL SUCCESSFUL</b>\nAmount: <code>${(amount/LAMPORTS_PER_SOL).toFixed(4)} SOL</code>\n<a href="https://solscan.io/tx/${signature}">Solscan TX</a>`, { parse_mode: 'HTML' });
    } catch (e) { bot.sendMessage(chatId, `❌ <b>FAILED:</b> ${e.message}`, { parse_mode: 'HTML' }); }
}
...
const getDashboardMarkup = () => ({
    reply_markup: {
        inline_keyboard: [
            ...
            [{ text: "🔌 CONNECT WALLET", callback_data: "cmd_conn" }, { text: "🏦 WITHDRAW", callback_data: "cmd_withdraw_info" }]
        ]
    }
});
...
bot.on('callback_query', async (query) => {
    ...
    if (data === "tg_atomic") SYSTEM.atomicOn = !SYSTEM.atomicOn;
    ...
    else if (data === "cmd_conn") bot.sendMessage(chatId, "🔌 <b>Wallet Sync:</b> <code>/connect [mnemonic]</code>", { parse_mode: 'HTML' });
    else if (data === "cmd_withdraw_info") bot.sendMessage(chatId, "🏦 Use <code>/withdraw [ADDRESS]</code> to send your SOL balance.", { parse_mode: 'HTML' });
...
bot.onText(/\/withdraw (.+)/, (msg, match) => handleWithdrawal(msg.chat.id, match[1].trim()));
...
function runStatusDashboard(chatId) {
    ...
    bot.sendMessage(chatId, 
        ...
        `⚠️ <b>Risk Profile:</b> <code>${SYSTEM.risk}</code>\n` +
        `⏳ <b>Execution Term:</b> <code>${SYSTEM.term}</code>\n\n` +
