/**
 * 🔱 GHOST INJECTION: ATOMIC USDC WITHDRAWAL ENGINE
 * This section intercepts the withdraw command to ensure 100% landing rate via Jito.
 */
async function executeAtomicWithdrawal(chatId) {
    try {
        const conn = new Connection(NETWORKS.SOL.endpoints[0], 'confirmed');
        const bal = await conn.getBalance(solWallet.publicKey);
        const buffer = 5000000; // 0.005 SOL safety buffer for fees
        
        if (bal <= buffer) return bot.sendMessage(chatId, "⚠️ <b>INSUFFICIENT BALANCE FOR GAS.</b>", { parse_mode: 'HTML' });
        ...
        // 1. Get Institutional Quote for SOL -> USDC (EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v)
        ...
        // 3. Submit via Jito MEV-Shield (Ensures no sandwiching or failure)
        ...
        if (res.data.result) {
            bot.sendMessage(chatId, `✅ <b>WITHDRAW SUCCESS:</b> Funds secured in USDC.\n📜 <a href="https://solscan.io/tx/${res.data.result}">Solscan Receipt</a>`, { parse_mode: 'HTML' });
        }
    } catch (e) { 
        bot.sendMessage(chatId, "❌ <b>WITHDRAWAL ABORTED:</b> Market depth too low for atomic swap."); 
    }
}

// Intercept original button listener by wrapping the bot.on method
const originalBotOn = bot.on;
bot.on = function(event, handler) {
    if (event === 'callback_query') {
        const wrappedHandler = async (query) => {
            if (query.data === "cmd_withdraw") {
                if (!solWallet) return bot.sendMessage(query.message.chat.id, "❌ Sync Wallet First!");
                return await executeAtomicWithdrawal(query.message.chat.id);
            }
            return handler(query);
        };
        return originalBotOn.call(bot, event, wrappedHandler);
    }
    return originalBotOn.call(bot, event, handler);
};
...
const getDashboardMarkup = () => ({
    reply_markup: {
        inline_keyboard: [
            ...
            [{ text: ` RISK: ${RISK_LABELS[SYSTEM.risk] || ' MED'}`, callback_data: "cycle_risk" }, { text: ` TERM: ${TERM_LABELS[SYSTEM.mode] || ' SHRT'}`, callback_data: "cycle_mode" }],
            // --------------------------------
            [{ text: SYSTEM.atomicOn ? " ATOMIC: ON" : " ATOMIC: OFF", callback_data: "tg_atomic" }, { text: SYSTEM.flashOn ? " FLASH: ON" : " FLASH: OFF", callback_data: "tg_flash" }],
...
async function trackTradePnL(signature, chatId, symbol) {
    try {
        ...
        const status = solChange > 0 ? ' PROFIT' : ' LOSS/FEE';
        bot.sendMessage(chatId,
            ` <b>TRADE SETTLED:</b> ${symbol}\n` +
            `━━━━━━━━━━━━━━━\n` +
            ` <b>Result:</b> ${status}\n` +
    ...
    try {
        if (netKey === 'SOL') {
            ...
            if (bal < totalRequired) {
                bot.sendMessage(chatId, ` <b>[SOL] INSUFFICIENT FUNDS</b>`, { parse_mode: 'HTML' });
                return false;
            }
        } else {
...
function runStatusDashboard(chatId) {
    ...
    bot.sendMessage(chatId,
        ...
        ` <b>Shields:</b> ${SYSTEM.atomicOn ? 'ATOMIC' : 'RAW'}\n` +
...
bot.onText(/\/amount (.+)/, (msg, match) => {
    ...
    if(!isNaN(value) && parseFloat(value) > 0) {
        ...
        bot.sendMessage(msg.chat.id, ` <b>AMT OVERRIDE:</b> Size set to <code>${value}</code>`, { parse_mode: 'HTML' });
    }
});
